...
final class Block {
	...
	public static function register_admin( $block = '', $args = array() ) {
		...
		wp_register_script(
			'above-the-fold-audit-' . $block_name . '-blocks',
	...
	public static function register( $block = '', $args = array() ) {
		...
		// Set editor styles.
		$editor_style = 'above-the-fold-audit-' . $block_name . '-blocks';
		if ( ! empty( $args['editor_style'] ) ) {
			$editor_style = $args['editor_style'];
		}

		register_block_type(
			'above-the-fold-audit/' . $block_name,
			array(
				'style'         => $styles,
				'script'        => $script,
				'editor_style'  => $editor_style,
				'editor_script' => 'above-the-fold-audit-' . $block_name . '-blocks',
			)
		);
	}
}
